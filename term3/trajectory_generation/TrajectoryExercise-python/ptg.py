...
def PTG(start_s, start_d, target_vehicle, delta, T, predictions):
    ...
    # 对到达终点的T时刻之前，4×timestep这点时间，根据参考汽车的运动预测，结合高斯分布的N个采样以及delta偏移，得到自动驾驶汽车的N个目标位置goal state
    all_goals = []
    timestep = 0.5
    t = T - 4 * timestep
    while t <= T + 4 * timestep:
        target_state = np.array(target.state_in(t)) + np.array(delta) #参考汽车的运动预测
    ...
    # 根据上述位置，求到达各个目标位置的轨迹的系数，用多项式拟合的方式
    trajectories = []
