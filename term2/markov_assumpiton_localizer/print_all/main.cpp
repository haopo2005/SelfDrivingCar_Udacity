...
int main() {
    ...
    ******************************************************************************/
	...
	{
		std::cout<<"id_i: "<<map_1d.landmark_list[i].id_i<<", x_f: "<<map_1d.landmark_list[i].x_f<<std::endl;;
	}

	for(int i=0;i<measurement_pack_list.size();i++)
	{
		MeasurementPackage temp = measurement_pack_list[i];
		std::cout<<"control_s_:"<<temp.control_s_.delta_x_f<<std::endl;
		for(int j=0;j<temp.observation_s_.distance_f.size();j++)
		{
			std::cout<<"distance_f:"<<temp.observation_s_.distance_f[j]<<std::endl;
		}
	}
	return 0;
}