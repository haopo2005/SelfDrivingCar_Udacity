...
#此处物理含义，以两边道路线作为圆弧的圆的半径，曲率半径
#由于定义了像素的长度，因此单位是米
#视频拍摄绕udacity公司附近开的，如果曲率半径大于10KM或者小于100米，那就有问题了
...
def findfit(binary_warped):
    ...
    if left_lane.detect == False: 
        ...
        left_lane.detect = True
        left_lane.center_prev = leftx_current
    else:
        if abs(leftx_base - left_lane.center_prev) > 50:
            leftx_current = left_lane.center_prev
        else:
            leftx_current = leftx_base       
    
    if right_lane.detect == False:
        rightx_current = rightx_base
        right_lane.detect = True
        right_lane.center_prev = rightx_current
    else:
        if abs(rightx_base - right_lane.center_prev) > 50:
            rightx_current = right_lane.center_prev
        else:
            rightx_current = rightx_base
...
def process_image(image):
    ...
    left_fit, right_fit = findfit(binary_warped)
...
white_output = 'harder_challenge_result_video.mp4'
